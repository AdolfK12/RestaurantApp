...
export const deleteItemMiddleware = (itemId) => async (dispatch) => {
  try {
    const response = await fetch(`${BASE_URL}/items/delete/${itemId}`, {
      method: "DELETE",
      headers: {
        access_token: localStorage.getItem("access_token"),
      },
  ...
  (categoriesId) => async (dispatch) => {
    try {
      const response = await fetch(
        `${BASE_URL}/categories/delete/${categoriesId}`,
        {
          method: "DELETE",
          headers: {
            access_token: localStorage.getItem("access_token"),
...
export const registerMiddleware = (newUserDetails) => async (dispatch) => {
  try {
    const response = await fetch(`${BASE_URL}/register`, {
      method: "POST",
      headers: {
